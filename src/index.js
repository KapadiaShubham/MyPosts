  ...
  createItem(name) {
    ...
    edit.addEventListener('click', () => {
      ...
      if (input.classList.contains('editing')) {
        edit.innerHTML = `<i class="fas fa-check" style="color:blue"></i>`;
      }
      else {
        edit.innerHTML = `<i class="fas fa-user-edit"></i>`;
      }
...
function LIKE(j) {
  ...
  if (arr[j].value[0] == 0) {
    ...
    arr[j].value[2] = 0;
    arr[j].value[1] += 1;
    like[j].innerHTML = `<i class="fas fa-thumbs-up"></i><div class="likes">${arr[j].value[1]}</div>`
    dislike[j].innerHTML = `<i class="far fa-thumbs-down"></i><div class="dislikes">${arr[j].value[3]}</div>`
  }
}
function DISLIKE(j) {
  let like = document.querySelectorAll('.like')
  ...
  if (arr[j].value[2] == 0) {
    ...
    arr[j].value[0] = 0;
    arr[j].value[3] += 1;
    dislike[j].innerHTML = `<i class="fas fa-thumbs-down"></i><div class="dislikes">${arr[j].value[3]}</div>`
    like[j].innerHTML = `<i class="far fa-thumbs-up"></i><div class="likes">${arr[j].value[1]}</div>`
  }
}
